...
UI for the Practice tab of the Shadowing English app (R0).

This module defines a PracticeTab class which:

- Shows the Practice table:
    columns: No, Content, Hide, Show
- Shows the Dictionary panel (word + meaning_vi).
- Provides playback controls & speed buttons (0.5x, 0.75x, 1.0x, 1.2x, 1.5x).
- Integrates with a WaveformView for highlighting current sentence.
...
The owner is typically the main application class.
"""
...
class PracticeTab:
    ...
    ----------
    parent : tk.Frame
        Parent container into which this Practice tab will place its frame.
    owner : Any
        An object that exposes at least:
    ...
    def _create_practice_table(self, parent: tk.Frame) -> None:
        """
        Create the practice table on the left side.
        """
    ...
    def _create_dictionary_panel(self, parent: tk.Frame) -> None:
        """
        Create the dictionary panel on the right.
        """
    ...
    def refresh_all(self) -> None:
        """
        Refresh practice table, dictionary, current sentence button,
        speed buttons, and waveform.
        """
    ...
    # ------------------------------------------------------------------ #
    # Event handlers – Practice table
    # ------------------------------------------------------------------ #

    def _on_practice_click(self, _event: tk.Event) -> None:
        ...
        if 0 <= idx < len(self.sentences):
            ...
            # Toggle hide/show if clicked on Hide/Show columns (optional)
            values = self.practice_tree.item(sel[0], "values")
            # values → (No, Content, Hide, Show)
            # Check which column was clicked via event.x? (complex)
            # For now, we only update selection and UI:
            self.refresh_all()

    def _on_practice_double_click(self, event: tk.Event) -> None:
    ...
    def play_current_sentence(self) -> None:
        ...
        - Only shows messagebox with duration & text if Begin/End is valid.
        In the future this should call an AudioController.
        """
        ...
        messagebox.showinfo(
            ...
            f"{s.original_text[:60]}...",
        )

    def toggle_pause(self) -> None:
        self.is_playing = not self.is_playing
        print("Playback (Practice):", "Playing" if self.is_playing else "Paused")
    ...
    def set_speed(self, speed: float) -> None:
        """
        Update owner's playback_speed and highlight the selected speed button.
        """
        self.playback_speed = speed
        self._update_speed_buttons()
        print(f"Playback speed (Practice) set to {speed}x")
